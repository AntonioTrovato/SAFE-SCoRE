...
class Entity:
    ...
    def is_parked(self) -> bool:
        ...
        An entity that was given a destination is never parked, whatever its
        recorded speed: scenarios that script motion with a goal (rather than
        with a recorded trajectory or an explicit speed) carry no speed data
        at all, and without this they would all convert to a scenario in which
        nothing moves.
        """
...
# CARLA renamed most vehicle blueprints in 0.9.12; the SCTrans recordings
# still carry the older 0.9.10-era names, which no longer resolve (Scenic's
# own oldBlueprintNames table only maps new -> old, so it cannot help here).
LEGACY_BLUEPRINTS = {
    ...
    "vehicle.mercedesccc.mercedesccc": "vehicle.mercedes.coupe",
    "vehicle.mercedes-benz.coupe": "vehicle.mercedes.coupe",
    "vehicle.mini.cooperst": "vehicle.mini.cooper_s",
    "vehicle.bmw.isetta": "vehicle.micro.microlino",  # removed; nearest microcar
}

_YEAR_SUFFIX_RE = re.compile(r"^(vehicle\.[^.]+\.[a-z_]*[a-z])(\d{4})$")


def normalize_blueprint(name: str) -> str:
    """Map a legacy CARLA blueprint name onto its current equivalent.

    Falls back to the documented rename pattern (a trailing four-digit year
    gets an underscore: `mkz2017` -> `mkz_2017`) for names not in the table.
    Unknown names are returned unchanged - CARLA will report them at spawn
    time, which is clearer than silently substituting a different vehicle.
    """
    if not name:
        return name
    if name in LEGACY_BLUEPRINTS:
        return LEGACY_BLUEPRINTS[name]
...
class XOSCParser:
    ...
    # Every vehicle category maps to Car: the CARLA blueprint carried over
    # from the .xosc (plus the recorded dimensions) is what decides the actual
    # vehicle. Pedestrian is the one class that genuinely behaves differently.
    SCENIC_CLASS = {
        "car":        "Car",
        "van":        "Car",
        "truck":      "Car",
    ...
    def _parse_entities(self):
        ...
        for so in find_all(ent_root, "ScenarioObject"):
            ...
            if veh is not None:
                ...
                e.blueprint = normalize_blueprint(attr_str(veh, "name"))
                bbox_src = veh
    ...
    def _parse_stories(self):
        for mg in find_all(self.root, "ManeuverGroup"):
            ...
            # Destination and target speed are only read from Init/Private
            # above. Plenty of scenarios declare them in the storyboard
            # instead - CARLA's own scenario_runner examples do - and without
            # this the entity looks motionless and converts to a parked one.
            # Init wins where both are present; this only fills the gaps.
            acq = find_first(mg, "AcquirePositionAction")
            if acq is not None:
                pos = self._read_world_position(acq)
                if pos is not None:
                    for name in actors:
                        e = self._entities.get(name)
                        if e is not None and e.goal is None:
                            e.goal = pos

            speed_node = find_first(mg, "AbsoluteTargetSpeed")
            if speed_node is not None:
                value = attr_float(speed_node, "value")
                if value > PARKED_SPEED_THRESHOLD:
                    for name in actors:
                        e = self._entities.get(name)
                        if e is not None and e.init_speed <= 0.0:
                            e.init_speed = value
...
class ScenicRulesWriter:
    ...
    def _map_and_model(self):
        ...
        # These four params must precede the model line: a model never
        # overrides a parameter the scenario has already set.
        self._w("param carla_map = None  # not a CARLA town - CARLA builds "
                "the world from the .xodr above")
        self._w("param use2DMap = True  # recorded data is 2D; no --2d "
                "needed on the CLI")
        self._w("param snapToGroundDefault = True  # sit actors on the road "
                "surface (the recording has no z)")
        self._w("param timeout = 60  # s - generating the world from "
                "OpenDRIVE outlasts the 10 s default")
        ...
        self._w("#   scenic <this file> --simulate --2d -p render 0 "
                "-p real_time 0   # headless, full speed")
        self._w("# --2d is optional (it runs without it) but ~8x faster: "
                "Scenic's 3D geometry")
        self._w("# checks dominate every step, and real-time playback "
                "cannot keep up without it.")
        self._w()
    ...
    def _relative_map_path(self) -> str:
        ...
        try:
            ...
            # Windows: relpath refuses to relate paths on different drives
            # (map on D:, output folder on C:). An absolute path is just as
            # valid inside localPath(), so fall back to it.
            rel = xodr
        return rel.replace("\\", "/")
    ...
    def _has_initial_speed(self, e: Entity) -> bool:
        """Whether `e` spawns with a recorded speed worth restoring."""
        return (e.spawn is not None and not e.is_pedestrian
                and not e.is_parked
                and e.init_speed > PARKED_SPEED_THRESHOLD)

    def _restore_initial_speed(self, e: Entity, indent: str = "    "):
        if self._has_initial_speed(e):
            self._w(f"{indent}# CARLA spawns at rest; restore the recorded "
                    f"speed on the first step")
            self._w(f"{indent}take SetSpeedAction(self.initialSpeed)")
    ...
    def _agent_behaviors(self):
        ...
        if False:  # Autoware target: no ego behavior is generated.
            ...
            self._restore_initial_speed(ego)
            self._w("    try:")
            if has_route:
                ...
                self._w("        do FollowLaneBehavior("
                        "target_speed=EGO_SPEED)")
            # In-lane rather than omnidirectional: withinDistanceToAnyObjs
            # also fires for vehicles parked beside or behind the ego, and
            # in dense recorded traffic something is always within range,
            # which brakes the ego to a permanent standstill.
            self._w("    interrupt when withinDistanceToObjsInLane(self, "
                    "EGO_SAFETY_DIST):")
    ...
    def _entities(self):
        ...
        self._w("# (CARLA still refuses to spawn actors that intersect).")
        self._w()
    ...
    def _real_time_pacing(self):
        """Scenic's CARLA interface steps as fast as the machine allows, so
        playback looks several times too fast. MetaDrive's interface paces
        itself; this gives the CARLA scenarios the same behavior."""
        self._banner("REAL-TIME PLAYBACK")
        w = self._w
        w("# Holds each step to one timestep of wall-clock time so the run")
        w("# plays at real speed, the way the MetaDrive interface does.")
        w("# Turn it off for batch runs:  -p real_time 0")
        w("import time as _time")
        w()
        w("monitor RealTimePacing():")
        w("    paced = bool(globalParameters.real_time)")
        w("    step = float(globalParameters.timestep)")
        w("    last = _time.perf_counter()")
        w("    while True:")
        w("        if paced:")
        w("            slack = step - (_time.perf_counter() - last)")
        w("            if slack > 0:")
        w("                _time.sleep(slack)")
        w("        last = _time.perf_counter()")
        w("        wait")
        w()
        w("require monitor RealTimePacing()")
        w()
