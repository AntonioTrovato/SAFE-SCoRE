"""
SCENARIO: PedestrianCrossingFront.scenic
SOURCE FILE: PedestrianCrossingFront.xosc (SCTrans)
SOURCE HEADER: CARLA:PedestrianCrossing
ENTITIES: ego + 0 moving NPC(s), 0 parked, 1 pedestrian(s)
...
so Autoware starts the ego at the recorded pose rather than its own default:
    [150.0000, 55.0000, 0.0, 0.0, 0.0, 180.0000]
"""
...
param weather_rain_intensity = 0.20
...
egoSpawnPt = new OrientedPoint at (150.0000, 55.0000), facing 90.00 deg
adversarySpawnPt = new OrientedPoint at (110.0000, 52.0000), facing -0.00 deg
...
adversary = new Pedestrian at adversarySpawnPt, facing adversarySpawnPt.heading,
